...
| 6 | Update `ComputeInstanceEventData` on heartbeat ingest to carry `instance_type` (if the metering-collector CloudEvent schema is going to be revisited anyway) so heartbeat-only paths can also resolve via catalog | `internal/api/handler.go` | Small |
